...
public class FormulaireProfAdmin_wt extends WDialog {
	...
	private WLabel ar,dates,nbCours,matieres,promos,nbProf;
	...
	public void afficher() {
		...
		nbProf = new WLabel("Nombre de profs total : "+edtAppli.getControleurDb().getNbProfs());
		grid.addWidget(nbProf); 
			...
			ok.clicked().addListener(ok, new Signal.Listener() {
				public void trigger() {
					...
					if(!edtAppli.getControleurDb().effacerProf(nomsProfs.get(prof),tf_ar.getValue())) {
						...
						nbProf.setText("Nombre de profs total : "+edtAppli.getControleurDb().getNbProfs());
						action("mettreAJour");
