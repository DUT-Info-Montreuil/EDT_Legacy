...
public class FormulaireMatiereAdmin_wt extends WDialog {
	...
	private WLabel dates,profs,promos,nbCours,ar,nbMatieres;
	...
	public void afficher() {
		...
		nbMatieres = new WLabel("Nombre de matieres total : "+edtAppli.getControleurDb().getNbMatieres());
		grid.addWidget(nbMatieres); 
			...
			ok.clicked().addListener(ok, new Signal.Listener() {
				public void trigger() {
					...
					if(!edtAppli.getControleurDb().effacerMatiere(nomsMatieres.get(matiere),tf_ar.getValue())) {
						...
						nbMatieres.setText("Nombre de matieres total : "+edtAppli.getControleurDb().getNbMatieres());
						action("mettreAJour");
