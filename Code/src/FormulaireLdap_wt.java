...
public class FormulaireLdap_wt extends WDialog {
		...
		try {
			...
			try {
				...
				while (values.hasMoreElements()){
					...
					String login = Objects.toString(attribs.get("uid"));
					if(login!=null) {
						if(login!="") {
							if(login.startsWith("uid: ")) {
					...
					if((Year.now().getValue()-Integer.parseInt(date))<=dateDiff) {
						if(login!=null) {
							if(!allEtudiantsPromo.contains(login.toLowerCase())) {
								if(nom!=null && prenom!="null") {
									if(prenom!=null && prenom!="null") {
										Etudiant e = new Etudiant(i,prenom,nom,login);
										etudiants.put(nom+" "+prenom,e);
	...
	public void action() {    
			...
			if(listeGroupesEtudiant.get(i)!=null) {
				if (allEtudiantsPromo.contains(listeGroupesEtudiant.get(i).getLogin().toLowerCase())) {
					...
					if(idE!=0) {
						...
						//System.out.println(listeGroupesEtudiant.get(i).getLogin());
		...
		Utilitaire.showMessageDialog("Elèves AJouter :", sl.toString());
		afficher();
	}    
}
