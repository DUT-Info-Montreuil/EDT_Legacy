...
public class ControleurDB {
	...
	public TreeMap<Integer,String> getPromotions3(int ar) {
		TreeMap<Integer,String> liste = new TreeMap<Integer,String>();
		Statement requeteRecherche;	
		try {					
			requeteRecherche = db.createStatement();
			ResultSet resultatTemporaire = requeteRecherche.executeQuery("SELECT id_promo,promo FROM promo");
			while (resultatTemporaire.next()) {
				if(checkPromo2(resultatTemporaire.getInt("id_promo"), ar)){
					liste.put(resultatTemporaire.getInt("id_promo"),resultatTemporaire.getString("promo"));
				}
			}
			resultatTemporaire.close();
	...
	public TreeMap<Integer,String> getProfs3(int ar) {
		TreeMap<Integer,String> liste = new TreeMap<Integer,String>();
		Statement requeteRecherche;	
		try {					
			requeteRecherche = db.createStatement();
			ResultSet resultatTemporaire = requeteRecherche.executeQuery("SELECT id_prof,nom_prof,prenom_prof FROM prof");
			while (resultatTemporaire.next()) {
				if(checkProf2(resultatTemporaire.getInt("id_prof"), ar)){
					liste.put(resultatTemporaire.getInt("id_prof"),resultatTemporaire.getString("nom_prof")+" "+resultatTemporaire.getString("prenom_prof"));
				}
			}
			resultatTemporaire.close();
			requeteRecherche.close();
	...
	public TreeMap<Integer,String> getMatieres3(int ar) {
		TreeMap<Integer,String> liste = new TreeMap<Integer,String>();
		Statement requeteRecherche;	
		try {					
			requeteRecherche = db.createStatement();
			ResultSet resultatTemporaire = requeteRecherche.executeQuery("SELECT id_matiere,nom_matiere FROM matiere");
			while (resultatTemporaire.next()) {
				if(checkMatiere2(resultatTemporaire.getInt("id_matiere"), ar)){
					liste.put(resultatTemporaire.getInt("id_matiere"),resultatTemporaire.getString("nom_matiere"));
				}
			}
			resultatTemporaire.close();
			requeteRecherche.close();
	...
	public int getNbPromos() {
		int nbPromos = 0;
		Statement requeteRecherche;
		try {
			requeteRecherche = db.createStatement();
			ResultSet resultat = requeteRecherche.executeQuery("SELECT count(*) from promo");
			if(resultat.next()) {
				nbPromos=resultat.getInt("count");
			}
			resultat.close();
			requeteRecherche.close();
		} catch (SQLException e) {
	...
	public int getNbProfs() {
		...
		try {
			requeteRecherche = db.createStatement();
			ResultSet resultat = requeteRecherche.executeQuery("SELECT count(*) from prof");
			if(resultat.next()) {
				nbProfs=resultat.getInt("count");
			}
			resultat.close();
			requeteRecherche.close();
		} catch (SQLException e) {
	...
	public int getNbMatieres() {
		...
		try {
			requeteRecherche = db.createStatement();
			ResultSet resultat = requeteRecherche.executeQuery("SELECT count(*) from matiere");
			if(resultat.next()) {
				nbMatieres=resultat.getInt("count");
			}
			resultat.close();
			requeteRecherche.close();
		} catch (SQLException e) {
	...
	public String ajouterPlageHoraire(PlageHoraire plage) {
		...
		try {
			...
			if (salleLibre && salle2Libre && profLibre && prof2Libre && prof3Libre && prof4Libre && groupeLibre) {
				String requete="INSERT INTO dispense VALUES (" + getIdPlageHoraire() + "," + dateDebut.getTimeInMillis() / 1000 + "," + 
					dateFin.getTimeInMillis() / 1000 + ",'" +  memo + "'," + idSalle + "," + idGroupe + "," + idMatiere + "," + idProf + ",'" + controle + "'," + 
					idSalle2 + "," + idProf2 + "," + idProf3 + "," + idProf4 + ",'f', '" + internet + "'," + idProfAppel + ")";
                boolean insertionReussie = (requeteRecherche.executeUpdate(requete) == 1);
				if (!insertionReussie)
					erreurInsertion += "Erreur d'insertion dans la base de données : " + Utilitaire.afficherDate(dateDebut, dateFin);
			} else if (!groupeLibre)
		...
		//return erreurInsertion+"<br>";
		return erreurInsertion;
	}
	
	
	public String ajouterPlageHoraire2(PlageHoraire plage) {
		String erreurInsertion = "";
		Statement requeteRecherche;
		try {
			requeteRecherche = db.createStatement();
	...
	public Boolean checkPromo2(int idP,int ar) {
		boolean date5ans=false;
		int nbModif;
		int date;
		String erreurInsertion = "";
		...
		try {
			...
			if (r.next()) {
				...
				tz.setEndRule(Calendar.OCTOBER, -1, Calendar.SUNDAY, 2 * 60 * 60 * 1000);
				dateDebut = new GregorianCalendar(tz);
				dateDebut.setTimeInMillis(r.getLong("date_fin") * 1000);
				if((Calendar.getInstance().get(Calendar.YEAR)-dateDebut.get(Calendar.YEAR))>=ar) {
					date5ans=true;
				}
			}else {
			...
			e.printStackTrace();
			erreurInsertion+=e.toString()+"\n";
			Utilitaire.showMessageDialog("ControleurDB : checkPromo2()", erreurInsertion.toString());
		}
		return date5ans;
	}
	
	public Boolean checkMatiere2(int idP,int ar) {
		boolean date5ans=false;
		int nbModif;
		int date;
		String erreurInsertion = "";
		...
		try {
			requeteRecherche = db.createStatement();
			ResultSet r = requeteRecherche.executeQuery("select date_fin from dispense where id_matiere="+idP+" order by date_fin desc");  
			if (r.next()) {
				GregorianCalendar dateDebut;
				SimpleTimeZone tz=new SimpleTimeZone(1*60*60*1000, "CET");
				tz.setStartRule(Calendar.MARCH, -1, Calendar.SUNDAY, 2 * 60 * 60 * 1000);
	...
	public Boolean checkProf2(int idP,int ar) {
		boolean date5ans=false;
		int nbModif;
		int date;
		String erreurInsertion = "";
		...
		try {
			requeteRecherche = db.createStatement();
			ResultSet r = requeteRecherche.executeQuery("select date_fin from dispense where id_prof="+idP+" order by date_fin desc");  
			if (r.next()) {
				GregorianCalendar dateDebut;
				SimpleTimeZone tz=new SimpleTimeZone(1*60*60*1000, "CET");
				tz.setStartRule(Calendar.MARCH, -1, Calendar.SUNDAY, 2 * 60 * 60 * 1000);
	...
	public Boolean effacerPromo2(int idP,int ar) {
		boolean date5ans=false;
		int nbModif;
		int date;
		String erreurInsertion = "";
		...
		try {
			...
			if(date5ans) {
				if(idP>0 ) {
					...
					nbModif = requeteRecherche.executeUpdate("DELETE from promo where id_promo='"+idP+"'");
					if (nbModif != 1)
						erreurInsertion+="Promotion Inexistante";
	...
	public Boolean effacerProf2(int prof,int ar) {
		boolean date5ans=false; 
		int nbModif;
		int date;
		String erreurInsertion = "";
		...
		try {
			requeteRecherche = db.createStatement();
			ResultSet r = requeteRecherche.executeQuery("select date_fin from dispense where id_prof="+prof+" order by date_fin desc");  
			if (r.next()) {
				GregorianCalendar dateDebut;
				SimpleTimeZone tz=new SimpleTimeZone(1*60*60*1000, "CET");
				tz.setStartRule(Calendar.MARCH, -1, Calendar.SUNDAY, 2 * 60 * 60 * 1000);
	...
	public Boolean effacerMatiere2(int mat,int ar) {
		boolean date5ans=false; 
		int nbModif;
		int date;
		String erreurInsertion = "";
		...
		try {
			requeteRecherche = db.createStatement();
			ResultSet r = requeteRecherche.executeQuery("select date_fin from dispense where id_matiere="+mat+" order by date_fin desc");  
			if (r.next()) {
				GregorianCalendar dateDebut;
				SimpleTimeZone tz=new SimpleTimeZone(1*60*60*1000, "CET");
				tz.setStartRule(Calendar.MARCH, -1, Calendar.SUNDAY, 2 * 60 * 60 * 1000);
	...
	public String getHistoriquePlageHoraire(PlageHoraire plage) {
		...
		dateDebut.setTimeInMillis(plage.getDateDebut().getTimeInMillis());
		dateDebut.set(Calendar.DAY_OF_MONTH, 1);
		while (dateDebut.get(Calendar.MONTH)!=Calendar.AUGUST)
		...
		try {
			requeteRecherche = db.createStatement();
			ResultSet resultat = requeteRecherche.executeQuery("SELECT SUM(date_fin-date_dbt) AS total FROM dispense WHERE id_matiere=" + idMatiere + " AND id_groupe=" + groupe.getIdGroupe() + " AND date_dbt>=" + (dateDebut.getTimeInMillis()/1000) + " AND date_fin<=" + dateFin);
			while (resultat.next())
		...
		//pas oublier quand on clic la plage disparait donc il faut ajouter le temps de la page en cours
		total+=(plage.getDateFin().getTimeInMillis()/1000)-(plage.getDateDebut().getTimeInMillis()/1000); 
		total/=60;
		int totalH=total/60;
		int totalM=total%60;
